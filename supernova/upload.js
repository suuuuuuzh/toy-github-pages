...
const GH_REPO = typeof siteConfig !== "undefined" ? siteConfig.ghRepo : "suuuuuuzh/toy-github-pages";
const GH_DIR = typeof siteConfig !== "undefined" ? siteConfig.ghDir : "expense-report/invoices/extra";
const GH_TOKEN_KEY = "gh-upload-token";
const LIB_DIR = typeof siteConfig !== "undefined" ? siteConfig.invoiceDir : "invoices/extra";
...
async function commitJob(job, onStatus) {
  ...
  job.files.forEach((en) => {
    if (remote.has(en.name)) {
      ...
      renameLibraryPath(LIB_DIR + "/" + en.name, LIB_DIR + "/" + name);
      en.name = name;
    }
    remote.add(en.name);
  });
...
async function migrateLocalUploads(onStatus) {
  ...
  const refMap = {};
  entries.forEach((en, i) => (refMap["upload:" + en[0]] = LIB_DIR + "/" + files[i].name));
